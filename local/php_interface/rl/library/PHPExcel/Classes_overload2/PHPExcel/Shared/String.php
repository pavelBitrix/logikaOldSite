...
class PHPExcel_Shared_String
{
    ...
    private static function buildControlCharacters()
    {
        for ($i = 0; $i <= 31; ++$i) {
            if ($i != 9 && $i != 10 && $i != 13) {
                $find = '_x' . sprintf('%04s', mb_orig_strtoupper(dechex($i))) . '_';
                $replace = chr($i);
    ...
    public static function getIsIconvEnabled()
    {
        ...
        if (defined('PHP_OS') && @mb_orig_stristr(PHP_OS, 'AIX') && defined('ICONV_IMPL') && (@strcasecmp(ICONV_IMPL, 'unknown') == 0) && defined('ICONV_VERSION') && (@strcasecmp(ICONV_VERSION, 'unknown') == 0)) {
            self::$isIconvEnabled = false;
            return false;
        }
    ...
    {
        if (mb_orig_strlen($str) < 2) {
            return $str;
        }
        $c0 = ord($str{0});
        $c1 = ord($str{1});
        if ($c0 == 0xfe && $c1 == 0xff) {
            $str = mb_orig_substr($str, 2);
    ...
    public static function Substring($pValue = '', $pStart = 0, $pLength = 0)
    {
        ...
        // else substr
        return mb_orig_substr($pValue, $pStart, $pLength);
    }
    ...
    public static function StrToUpper($pValue = '')
    {
        ...
        return mb_orig_strtoupper($pValue);
    }
    ...
    public static function StrToLower($pValue = '')
    {
        ...
        return mb_orig_strtolower($pValue);
    }
    ...
    public static function StrCaseReverse($pValue = '')
    {
        ...
        return mb_orig_strtolower($pValue) ^ mb_orig_strtoupper($pValue) ^ $pValue;
    }
    ...
    public static function SYLKtoUTF8($pValue = '')
    {
        ...
        if (mb_orig_strpos($pValue, '') === false) {
            return $pValue;
        }
    ...
    public static function testStringAsNumeric($value)
    {
        ...
        $v = floatval($value);
        return (is_numeric(mb_orig_substr($value, 0, mb_orig_strlen($v)))) ? $v : $value;
    }
}
