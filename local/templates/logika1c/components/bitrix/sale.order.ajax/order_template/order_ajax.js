...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
			...
			if (this.result.DELIVERY)
			{
				...
				if (this.options.deliveriesPerPage > 0 && this.result.DELIVERY.length > this.options.deliveriesPerPage)
				{
					...
					for (i = 0; i < this.result.DELIVERY.length; i++)
					{
						...
						if(this.result.DELIVERY[i].NAME == 'Самовывоз'){
							this.result.DELIVERY[i].PRICE_FORMATED = "Бесплатно";
						}
					}
		...
		activateRegion: function(deliveryName)
		{
			if (!this.regionBlockNode || !this.regionHiddenBlockNode)
				return;
			
			this.regionBlockNode.style.display = '';
			this.regionBlockNode.querySelector('[data-entity="section-title"]').innerHTML =
				'<span class="bx-soa-section-title-count"></span>' + BX.util.htmlspecialchars(deliveryName);

			if (BX.hasClass(this.regionBlockNode, 'bx-active'))
				return;
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				regionNode = BX.create('DIV', {props: {className: 'container'}});
				regionNodeCol = BX.create('DIV', {props: {className: 'col'}});	
				this.editRegionItems(regionNode);		
		...
		editRegionItems: function(propsNode)
		{
			if (!this.result.ORDER_PROP || !this.propertyCollection)
				return;

			var RegionItemsContainer = BX.create('DIV', {props: {className: 'row bx-soa-customer'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;
			if (!RegionItemsContainer)
				RegionItemsContainer = this.regionBlockNode.querySelector('.row.bx-soa-customer');
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					...
					{
						this.getPropertyRowNode(property, RegionItemsContainer, false);		
					}
				}
			}
			

			propsNode.appendChild(RegionItemsContainer);
		},
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				{
					this.editPropsComment(paySystemNode);
				}
		...
		createPaySystemItem: function(item)
		{
			...
			label = BX.create('DIV', {
				...
				children: [
					...
					BX.create('LABEL', {props: {className: 'bx-soa-pp-company-smalltitle'}, text: item.NAME, for: 'ID_PAY_SYSTEM_ID_' + paySystemId})
				]
			});
		...
		checkRegionShow: function()
		{
			var currentDelivery = this.getSelectedDelivery(), name;
			if (currentDelivery.STORE.length == 0)
			{
				...
				this.editSection(this.regionBlockNode);
			}
			else
			{
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'order-md-1 order-2 bx-soa-pp-item-container'}}),
		...
		createDeliveryItem: function(item)
		{
			...
			{
				labelNodes.push(BX.create('LABEL', {
					props: {tag: 'label', className: 'bx-soa-pp-company-smalltitle', for: 'ID_DELIVERY_ID_' + deliveryId},
					text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME, 
				}));
			}
			
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined' || typeof item.PRICE == 'undefined')
			{
				labelNodes.push(
					BX.create('DIV', {
		...
		getDeliverySortedArray: function(objDelivery)
		{
			...
			for (k in objDelivery)
			{
				if (objDelivery.hasOwnProperty(k))
				{
					...
					{
						if(objDelivery[k].NAME == 'Самовывоз'){
							objDelivery[k].PRICE_FORMATED = 'Бесплатно';
						}
		...
		isValidRegionBlock: function()
		{
			...
			var regionProps = this.orderBlockNode.querySelectorAll('textarea'),
				regionErrors = [],
		...
		showErrorTooltip: function(tooltipId, targetNode, targetInput, text)
		{
		...
		showValidationResult: function(inputs, errors)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				if (errors[i] && errors[i].length)
				{
					...
					BX.addClass(input, 'required');
				}
				else
				{
					BX.removeClass(inputDiv, 'has-error');
					BX.removeClass(input, 'required');
				}
			}

			// if (errors.length)
			// 	this.showErrorTooltip(tooltipId, label, input, errors.join('<br>'));
			// else
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'order-2 col-sm-12'}});
			label = BX.create('LABEL', {
		...
		editTotalBlock: function()
		{
			...
			if (this.result.DELIVERY.length && curDelivery.ID != 15)
			{
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-button-container' + (!showOrderButton ? ' d-none' : '')},
						children: [
							BX.create('A', {
								props: {
									href: 'javascript:void(0)',
		...
		editMobileTotalBlock: function()
		{
			...
			//this.mobileTotalBlockNode.appendChild(this.basketBlockNode.cloneNode(true));
		...
		totalBlockScrollCheck: function()
		{
			...
			if (ghostBottom - this.basketBlockNode.offsetHeight < scrollTop + 20){
				BX.addClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				BX.addClass(this.basketBlockNode, 'bx-soa-cart-total-bottom');
			}
			else{
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-bottom');
				BX.removeClass(this.basketBlockNode, 'bx-soa-cart-total-bottom');
			}
			if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				BX.addClass(this.basketBlockNode, 'bx-soa-cart-total-fixed');
				this.totalGhostBlockNode.style.paddingTop = this.totalInfoBlockNode.offsetHeight + this.basketBlockNode.offsetHeight + 'px';
				this.basketBlockNode.style.width = width +'px';
				this.totalInfoBlockNode.style.top = this.basketBlockNode.offsetHeight + 20 + 'px';
			...
			else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				BX.removeClass(this.basketBlockNode, 'bx-soa-cart-total-fixed');
				this.totalGhostBlockNode.style.paddingTop = 0;
				this.totalInfoBlockNode.style.width = '';
				this.basketBlockNode.style.width='';
			}
		},

		totalBlockResizeCheck: function()
		{
			if ((!this.totalInfoBlockNode && !this.basketBlockNode) || !this.totalGhostBlockNode)
				return;
			...
			if (BX.hasClass(this.basketBlockNode, 'bx-soa-cart-total-fixed')){
				this.basketBlockNode.style.width = this.totalGhostBlockNode.offsetWidth + 'px';
			}
		},
