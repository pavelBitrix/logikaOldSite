...
<script>
// Получаем все элементы с классом "product-item-price-current"
var elements = document.getElementsByClassName("product-item-price-current");

// Перебираем элементы
for (var i = 0; i < elements.length; i++) {
  // Проверяем, является ли содержимое элемента пустым
  if (elements[i].innerHTML.trim() === "") {
    // Если пусто, добавляем текст "Под заказ"
    elements[i].innerHTML = "Под заказ";
  }
}
</script>